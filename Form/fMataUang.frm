...
Dim SimpanDef As Boolean
Private Sub IkatData()
On Error GoTo aa
Set aSet = aData.AmbilData("SELECT * FROM MataUang")
...
Grid.Columns(2).ValueItems.Presentation = dbgCheckBox
...
'Set fRpt.Report = Lap.GetKonsumen
'fRpt.Report.Database.SetDataSource aSet, 3
...
If ColIndex = 2 Then SimpanDef = True
End Sub

Private Sub Grid_AfterUpdate()
If SimpanDef Then
aData.ExecCommand "Update matauang set matauang.default=0 " & _
"Where [Kode Mata Uang]<>'" & Grid.Columns(0).Value & "'"
Call IkatData
Call IkatGrid
End If
SimpanDef = False
...
If ColIndex = 2 And Grid.Columns(2).Value = True Then Cancel = True
End Sub
