...
Begin VB.Form fKonsumen 
   Caption         =   "Master Konsumen"
   ClientHeight    =   9855
   ClientLeft      =   600
   ClientTop       =   1065
   ClientWidth     =   13800
...
On Error GoTo aa
'Set aSet = aData.AmbilData("SELECT Kode, Nama, Alamat, Kota, Telepon, [Credit Limit],Salesman,[Lebih Bayar] FROM Konsumen")
Set aSet = aData.AmbilData("SELECT Konsumen.Kode, Konsumen.Nama, " & _
"Konsumen.Alamat, Konsumen.[Credit Limit], Konsumen.[Lama Credit],Konsumen.[Wilayah], " & _
"Konsumen.Kota, Konsumen.Telepon, Konsumen.Ket, Konsumen.Salesman, Konsumen.[Lebih Bayar], [InputPiutang Query].[Sum Of Sisa] as [Jumlah Piutang] FROM Konsumen left JOIN [InputPiutang Query] ON Konsumen.Kode = [InputPiutang Query].Kepada;")
...
On Error GoTo aa
Dim rsKod As New ADODB.Recordset
Dim aaa As New TrueOleDBGrid80.ValueItem
Grid.Columns(0).Width = 1200
Grid.Columns(1).Width = 2300
Grid.Columns(2).Width = 3000
Grid.Columns(3).Width = 1800
Grid.Columns(3).NumberFormat = "Standard"
Grid.Columns(4).Width = 900
Grid.Columns(5).Width = 1500
Dim oRS2 As New ADODB.Recordset
Set oRS2 = aData.AmbilData("SELECT * from Wilayah;")
Dim i As Integer, Oi As New ValueItem
For i = 1 To oRS2.RecordCount
  Oi.Value = oRS2!Kode:    Oi.DisplayValue = oRS2!Nama
  Grid.Columns(5).ValueItems.Add Oi
  oRS2.MoveNext
Next i
Grid.Columns(5).AutoCompletion = True
Grid.Columns(5).AutoDropDown = True
Grid.Columns(5).ValueItems.Translate = True
Grid.Columns(5).ValueItems.Presentation = dbgComboBox

Grid.Columns(6).Width = 1500
Grid.Columns(7).Width = 1500
Grid.Columns(8).Width = 3500

Grid.Columns(9).Width = 1500
Grid.Columns(9).AutoCompletion = True
Grid.Columns(9).AutoDropDown = True
Grid.Columns(9).ValueItems.Translate = True
Grid.Columns(9).ValueItems.Validate = True
Grid.Columns(9).ValueItems.Presentation = dbgComboBox
If rsKod.State = adStateOpen Then rsKod.Close
Set rsKod = aData.AmbilCommand("Select Kode, Nama from Salesman")
Do While Not rsKod.EOF
aaa.Value = rsKod!Kode
aaa.DisplayValue = IIf(IsNull(rsKod!Nama), "", rsKod!Nama)
Grid.Columns(9).ValueItems.Add aaa
rsKod.MoveNext
Loop
Grid.Columns(10).Locked = True
Grid.Columns(11).Locked = True
Grid.Columns(11).NumberFormat = "Standard"
Grid.HeadLines = 2.5
Grid.Font.Bold = True
Grid.Font.Size = 10
Exit Sub
...
On Error GoTo aa
'Dim XaX As String
'frmLogin.Caption = "Customer Password"
'frmLogin.Jenis = "Cust"
...
'If RegC.HashString(XaX) = rsPass!PassCust Then
